...
export default createStore({
  ...
  actions: {
    ...
    async signup({ commit }, { email, username, password, workspace }) {
      try {
    ...
    async fetchWorkspaceUsers({ state, commit }) {
        try {
          const response = await axios.get(`${getUrlBase()}/users`, {
            headers: {
      ...
      async createNewChat({ state, commit }, { members, type, name, public: isPublic }) {
        try {
          ...
          if (uniqueMembers.length === 2 && !name) {
            ...
            chatType = isPublic ? 'publicChannel' : 'privateChannel';
          } else {
            chatType = 'group';
          }
